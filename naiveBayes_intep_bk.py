...
#print("testDf = ",testList)
	...
	resultOp.write("|".join(["pred_lbl","pred_prob","prior_prob","prior_lbl","max_comp_lbl","max_comp","max_comp_prob","unk"]) + "\n")
	logRes = []
	class_prob_log = {}
	...
	for i,x in enumerate(testList):
		...
		for c in classDict:
			...
			for w in x.split():
				...
				unkFlag = 0
				if wordDict[w][c] > maxComp[1]: 
					maxComp[0] = w
					maxComp[1] = wordDict[w][c]*class_prob[c]
		...
		resultOp.write("|".join([str(sorted_x[0][0]),str(sorted_x[0][1]),str(sorted_prior[0][1]),sorted_prior[0][0],str(maxLocsSorted[-1][0]),str(maxLocsSorted[-1][1]),str(maxLocsSorted[-1][2]),str(unkFlag)]) + "\n")
...
if __name__ == "__main__":
	...
	dataList = np.genfromtxt('timestamp_1day_hts',dtype='str')
