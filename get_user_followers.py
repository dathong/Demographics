...
def get_followers(userList):
    ...
    while totalIndex < len(userList):
       try:
          ...
          users = tweepy.Cursor(api.followers, screen_name=user_name, count=200).items()
 #        print("len users = ",len(users))
          #with open("user_followers/" + user_name + ".csv","w") as f:
          filename = "user_followers/" + user_name + ".csv"
          finishedList = "./user_followers/finishedList.csv"
...
print("started...")
userList = []
finishedList = pd.read_csv("./user_followers/finishedList.csv",header=None).values.tolist()
