...
if data_type in ["nouns","hash_tags_nouns"]:
...
test_time_stamp = ['2017-10-28','2017-10-29','2017-10-30']
#--get city list---
if not os.path.exists(processed_csv_file) or replace_flag:
	...
	df["hash_tags_nouns"] = df["hash_tags"].map(str) + "," + df["nouns"].map(str)
	df['hash_tags_nouns'] = df['hash_tags_nouns'].str.replace(",nan","").str.replace("nan,","")
	print("df shape = ",df.shape)
	print("df hash_tags_nouns = ",df[['hash_tags','nouns','hash_tags_nouns']])
	df['time_stamp'] = df['time_stamp'].str.split(" ").str[0].replace(" ","_")
	df = df[df['time_stamp'] >= date_cutoff]
	df = df[df.location.isin(locs)]
	df = df[df.time_stamp.isin(test_time_stamp)]
	#df = df[~df['nouns'].isnull()]
	#df = df[~df['hash_tags'].isnull()]
	df = df[(~df['nouns'].isnull()) | (~df['hash_tags'].isnull())]
...
if not os.path.exists(expFol + "/fold" + str(fold)) or replace_flag:
	...
	for f in range(10):
		...
		for date in dates:
			...
			if data_type == "hash_tags_nouns":
				df2trainX = dfTrainX[['hash_tags_nouns','location']]
				df2testX = dfTestX[['hash_tags_nouns','location']]
